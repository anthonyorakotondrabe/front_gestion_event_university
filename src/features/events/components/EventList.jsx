...
const EventList = ({ events, onEdit, isModeration = false }) => {
  ...
  if (!events || events.length === 0) {
    return (
      <div className="bg-white dark:bg-[#1f2028] p-12 rounded-3xl border border-dashed border-gray-200 dark:border-white/10 text-center">
        <p className="text-gray-500 dark:text-gray-400 font-medium">Aucun événement trouvé.</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
      {events.map((event) => (
        <div key={event.id_evenement} className="bg-white dark:bg-[#1f2028] rounded-3xl p-6 border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-xl transition-all group">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="flex items-center gap-3 text-gray-500 dark:text-gray-400">
              ...
              <ParticipantCount eventId={event.id_evenement} maxCapacity={event.capacite_max} />
            </div>
          </div>
